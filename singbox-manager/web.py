...
"""
import io, json, base64, logging, time, hashlib
...
# ── Кэш для QR-кодов ─────────────────────────────────────────────────────
_qr_cache = {}
_QR_CACHE_TTL = 3600  # секунд (1 час)
...
def qr_b64_cached(data: str) -> str:
    """Кэширует QR-коды на _QR_CACHE_TTL секунд."""
    key = hashlib.md5(f"{data}:{_QR_CACHE_TTL}".encode()).hexdigest()
    
    if key in _qr_cache:
        cached, ts = _qr_cache[key]
        if time.time() - ts < _QR_CACHE_TTL:
            return cached
    
    result = qr_b64(data)
    _qr_cache[key] = (result, time.time())
    
    if len(_qr_cache) > 200:
        oldest = min(_qr_cache.items(), key=lambda x: x[1][1])
        del _qr_cache[oldest[0]]
...
def index():
    ...
    for u in users:
        ...
        # QR-коды генерируем лениво через API, здесь не создаём
        ...
        # Безопасный ID для модальных окон
        u["safe_id"] = u["name"].lower().replace(" ", "-").replace("_", "-").replace(".", "-")
    ...
    return render_template(
        ...
        sys_stats=sys_stats,
        srv_stats={
            "total_rx": manager.fmt_bytes(srv_stats["total_rx"]),
            "total_tx": manager.fmt_bytes(srv_stats["total_tx"]),
            "started_at": (srv_stats.get("started_at") or "")[:10],
        },
...
@panel.route("/api/qr/<name>/<type>")
@auth.login_required
def get_qr(name, type):
    """Отдает QR-код лениво (только при запросе)."""
    ...
    data = None
    if type == 'uri':
        data = manager.build_hy2_uri(u)
    elif type == 'sub':
        data = manager.build_sub_url(u)
    
    if not data:
        return jsonify({"error": "invalid type"}), 400
        
    b64 = qr_b64_cached(data)
    return jsonify({"qr": b64})
...
@panel.route("/api/reset_expire/<name>", methods=["POST"])
@auth.login_required
def api_reset_expire(name):
    if not db.get_user_by_name(name):
        return jsonify({"error": "not found"}), 404
...
@panel.route("/api/reset_server_stats", methods=["POST"])
@auth.login_required
def api_reset_server_stats():
    db.reset_server_stats()
    return jsonify({"ok": True})


# ── Подписка и конфиг — без PREFIX и без auth ───────────────────────────
